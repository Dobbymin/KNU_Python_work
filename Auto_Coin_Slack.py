...
# 시작 메세지 슬랙 전송
post_message(myToken, channel_name, "autotrade start")
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            if target_price < current_price and ma15 < current_price:
                ...
                if krw > 5000:
                    ...
                    post_message(myToken, channel_name, {coin + "buy : "} +str(buy_result))
        else:
            btc = get_balance(coin)
            if btc > 0.00008:
                sell_result = upbit.sell_market_order({"KRW-" + coin}, btc*0.9995)
                post_message(myToken, channel_name, {coin + "sell : "} +str(sell_result))
        time.sleep(1)
